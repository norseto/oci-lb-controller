...
var (
	...
	setupReconciler = func(mgr managerFacade) error {
		...
		return (&controller.LBRegistrarReconciler{
			Client:   real.GetClient(),
			Scheme:   real.GetScheme(),
			Recorder: real.GetEventRecorderFor("lbregistrar-controller"),
		}).SetupWithManager(real)
	}
	addHealthChecks = func(mgr managerFacade, probeAddr string) error {
		real, ok := mgr.(ctrl.Manager)
		if !ok {
			return fmt.Errorf("invalid manager type")
		}
		if err := real.AddHealthzCheck("healthz", healthz.Ping); err != nil {
			return err
		}
		if err := real.AddReadyzCheck("readyz", healthz.Ping); err != nil {
			return err
		}
		return nil
	}
...
func disableHTTP2Proto(c *tls.Config) {
...
func run(args []string) int {
	...
	fs := flag.NewFlagSet("oci-lb-controller", flag.ContinueOnError)
	...
	if !enableHTTP2 {
		tlsOpts = append(tlsOpts, disableHTTP2Proto)
	}
	...
	if err := addHealthChecks(mgr, probeAddr); err != nil {
		setupLog.Error(err, "unable to set up health/ready checks")
		return 1
	}
